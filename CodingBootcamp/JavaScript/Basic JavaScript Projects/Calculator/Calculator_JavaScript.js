...
function Input_Digit(digit) {
    ...
    //Display_Value to the key that ws clicked.
...
function Input_Decimal(dot) {
    ...
    //we are saying that id the Display_Value does not contan a decimal point
...
function Handle_Operator(Next_Operator) {
    ...
    //displayed on the screen to a number and then store the resukt in
    ...
    } else if (operator) {//checks if an operator alredy exist
        const Value_Now = First_Operand || 0;
        //if operator exist, property lookup is performed for the operator 
        //in the Perform_Calculation object and the function that natches the 
        //operator is executed
        let result = Perform_Calculation[operator](Value_Now, Value_of_Input);
        //here we add a fixed amount of numbers after the decimal
        result = Number(result).toFixed(9)
        //this will remoive any trailing 0's
